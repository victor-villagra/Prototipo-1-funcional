...
function CalorieRing({ consumed, goal }) {
  ...
  return React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 20 } },
    React.createElement('svg', { width: 180, height: 180, viewBox: '0 0 180 180' },
      React.createElement('circle', { cx, cy, r, fill: 'none', stroke: '#EAE0D0', strokeWidth: 12 }),
      ...
      React.createElement('text', { x: cx, y: cy - 10, textAnchor: 'middle', fontFamily: "'Nunito',sans-serif", fontSize: 30, fontWeight: 900, fill: '#1E1408' }, consumed.toLocaleString()),
      React.createElement('text', { x: cx, y: cy + 10, textAnchor: 'middle', fontFamily: "'DM Sans',sans-serif", fontSize: 12, fill: '#7A6652' }, 'kcal consumidas'),
    ),
    React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: 12 } },
      React.createElement('div', null,
        React.createElement('div', { style: { fontSize: 11, color: '#9A8878', fontWeight: 500 } }, 'Meta'),
        React.createElement('div', { style: { fontFamily: "'Nunito',sans-serif", fontSize: 20, fontWeight: 800, color: '#1E1408' } }, (goal || 2000).toLocaleString()),
      ),
      React.createElement('div', null,
        React.createElement('div', { style: { fontSize: 11, color: '#9A8878', fontWeight: 500 } }, 'Restante'),
...
function MacroBar({ label, val, goal, color, bg, unit }) {
  const pct = Math.min((val / (goal || 1)) * 100, 100);
  const over = val > goal;
  return React.createElement('div', { style: { marginBottom: 10 } },
    React.createElement('div', { style: { display: 'flex', justifyContent: 'space-between', marginBottom: 5 } },
      React.createElement('span', { style: { fontSize: 13, fontWeight: 500, color: '#1E1408' } }, label),
      React.createElement('span', { style: { fontSize: 13, fontWeight: 600, color: over ? '#C03030' : '#7A6652', fontFamily: "'Nunito',sans-serif" } }, `${val.toFixed(0)} / ${goal}${unit}`)
    ),
    React.createElement('div', { style: { background: '#EAE0D0', borderRadius: 999, height: 8, overflow: 'hidden' } },
...
function MealCard({ meal, idx, onTap }) {
  ...
  return React.createElement('div', {
    ...
    React.createElement('div', { style: { flex: 1 } },
      React.createElement('div', { style: { fontSize: 15, fontWeight: 600, color: '#1E1408' } }, meal.name),
      React.createElement('div', { style: { fontSize: 12, color: '#9A8878', marginTop: 2 } },
        items.length > 0 ? items.slice(0, 2).join(' · ') : 'Sin alimentos'
      ),
    ),
    React.createElement('div', { style: { textAlign: 'right' } },
      ...
      React.createElement('div', { style: { fontSize: 11, color: '#9A8878' } }, 'kcal'),
    )
  );
}

function Dashboard({ onNavigate, userData, todayMeals, dailyGoals, unreadCount }) {
  ...
  const macros = [
    { label: 'Proteína', val: totals.protein, goal: goals.protein, color: '#7EC8E3', bg: '#DFF3FA', unit: 'g' },
    { label: 'Grasa',    val: totals.fat,     goal: goals.fat,     color: '#C5A3FF', bg: '#EFE4FF', unit: 'g' },
    { label: 'Carbos',   val: totals.carbs,   goal: goals.carbs,   color: '#FF8C69', bg: '#FFE4DB', unit: 'g' },
    { label: 'Azúcar',   val: totals.sugar,   goal: goals.sugar,   color: '#FFB3C6', bg: '#FFE8EF', unit: 'g' },
  ];
  ...
  return React.createElement('div', { style: { padding: '0 0 16px' } },
    React.createElement('div', { style: { padding: '16px 24px 0', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' } },
      React.createElement('div', null,
        React.createElement('div', { style: { fontSize: 14, color: '#7A6652', fontWeight: 500 } }, greeting + ','),
        React.createElement('div', { style: { fontFamily: "'Nunito',sans-serif", fontSize: 26, fontWeight: 800, color: '#1E1408', lineHeight: 1.2 } }, (userName || 'Usuario') + ' 👋'),
      ),
      React.createElement('div', {
        ...
        (unreadCount || 0) > 0 && React.createElement('div', {
          style: { position: 'absolute', top: -2, right: -2, minWidth: 18, height: 18, borderRadius: 999, background: '#FF6B6B', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 4px', border: '2px solid #FEFAF3' }
        },
          React.createElement('span', { style: { fontSize: 10, fontWeight: 800, color: 'white', lineHeight: 1 } }, unreadCount > 9 ? '9+' : unreadCount)
        )
      )
    ),
    React.createElement('div', { style: { margin: '16px 16px 0', background: 'white', borderRadius: 24, padding: '20px 16px', boxShadow: '0 2px 12px rgba(30,20,8,0.08)' } },
      React.createElement(CalorieRing, { consumed: Math.round(totals.kcal), goal: goals.kcal })
    ),
    React.createElement('div', { style: { margin: '14px 16px 0', background: 'white', borderRadius: 20, padding: '16px 18px', boxShadow: '0 2px 12px rgba(30,20,8,0.07)' } },
      React.createElement('div', { style: { fontFamily: "'Nunito',sans-serif", fontSize: 16, fontWeight: 700, color: '#1E1408', marginBottom: 14 } }, 'Macronutrientes'),
    ...
    React.createElement('div', { style: { margin: '14px 16px 0' } },
      React.createElement('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 } },
        React.createElement('div', { style: { fontFamily: "'Nunito',sans-serif", fontSize: 16, fontWeight: 700, color: '#1E1408' } }, 'Comidas de hoy'),
      ...
      (todayMeals && todayMeals.length > 0)
        ? todayMeals.map((m, i) => React.createElement(MealCard, { key: m.id || i, meal: m, idx: i, onTap: () => {} }))
        : React.createElement('div', {
            style: { background: 'white', borderRadius: 16, padding: '20px 16px', textAlign: 'center', boxShadow: '0 2px 12px rgba(30,20,8,0.07)', marginBottom: 10 }
          },
            React.createElement('div', { style: { fontSize: 32, marginBottom: 8 } }, '🍽️'),
            React.createElement('div', { style: { fontSize: 14, color: '#7A6652', fontWeight: 500 } }, 'Aún no has registrado comidas hoy'),
            React.createElement('div', { style: { fontSize: 12, color: '#9A8878', marginTop: 4 } }, 'Toca + para agregar tu primera comida')
          ),
      React.createElement('div', {
        onClick: () => onNavigate('add', ''),
        style: {
          border: '1.5px dashed #D4C8B4', borderRadius: 16, padding: '14px 16px',
        ...
        React.createElement('span', { style: { fontSize: 14, fontWeight: 600, color: '#7A6652' } }, 'Agregar comida')
      )
    )
  );
}
