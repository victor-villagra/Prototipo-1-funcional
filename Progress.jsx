...
function WeekChart({ weekData, goalKcal }) {
  ...
  return React.createElement('div', { style: { background: 'white', borderRadius: 20, padding: '18px 16px', boxShadow: '0 2px 12px rgba(30,20,8,0.08)', marginBottom: 14 } },
    React.createElement('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 } },
      React.createElement('div', null,
        React.createElement('div', { style: { fontFamily: "'Nunito',sans-serif", fontSize: 16, fontWeight: 700, color: '#1E1408' } }, 'Esta semana'),
        React.createElement('div', { style: { fontSize: 12, color: '#7A6652', marginTop: 2 } },
    ...
    React.createElement('div', { style: { position: 'relative', marginBottom: 12 } },
      React.createElement('div', { style: { display: 'flex', alignItems: 'flex-end', gap: 8, height: chartH + 24, position: 'relative' } },
        weekData.map((d, i) => {
          ...
          const barColor = !d.logged ? '#F5EFE4' : over ? '#FF8C69' : isToday ? '#F5D040' : '#FFE682';
          return React.createElement('div', { key: d.day, style: { flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 } },
            ...
            React.createElement('div', { style: { fontSize: 11, fontWeight: isToday ? 700 : 500, color: isToday ? '#F5C030' : '#9A8878' } }, d.day)
          );
        })
      ),
      ...
      React.createElement('div', {
        style: {
          ...
          right: 0,
          fontSize: 10, fontWeight: 600, color: '#9A8878',
          background: 'white', padding: '1px 6px', borderRadius: 6,
          border: '1px solid #EAE0D0', zIndex: 3,
        }
    ...
    React.createElement('div', { style: { display: 'flex', gap: 14, flexWrap: 'wrap' } },
      React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 5 } },
        ...
        React.createElement('span', { style: { fontSize: 11, color: '#9A8878' } }, 'En meta')
      ),
      React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 5 } },
        React.createElement('div', { style: { width: 12, height: 4, background: '#FF8C69', borderRadius: 2 } }),
        React.createElement('span', { style: { fontSize: 11, color: '#9A8878' } }, 'Excedido')
      ),
      React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 5 } },
        React.createElement('div', { style: { width: 14, height: 0, borderTop: '1.5px dashed #B8A898' } }),
        React.createElement('span', { style: { fontSize: 11, color: '#9A8878' } }, 'Meta diaria')
      ),
      React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 5 } },
        React.createElement('div', { style: { width: 12, height: 4, background: '#F5EFE4', borderRadius: 2, border: '1px dashed #D4C8B4' } }),
        React.createElement('span', { style: { fontSize: 11, color: '#9A8878' } }, 'Sin registro')
      )
    )
  );
}
...
function HistoryRow({ entry, goal }) {
  ...
  return React.createElement('div', {
    style: { background: 'white', borderRadius: 14, padding: '12px 14px', display: 'flex', alignItems: 'center', gap: 12, boxShadow: '0 1px 8px rgba(30,20,8,0.06)', marginBottom: 8 }
  },
    React.createElement('div', { style: { flex: 1 } },
      React.createElement('div', { style: { fontSize: 14, fontWeight: 600, color: '#1E1408' } }, entry.date),
      React.createElement('div', { style: { fontSize: 12, color: '#9A8878', marginTop: 2 } }, `${entry.kcal.toLocaleString()} kcal · ${Math.round((entry.kcal / (goal || 2000)) * 100)}% de meta`)
    ),
...
function Progress({ dailyLog, dailyGoals }) {
  ...
  return React.createElement('div', { style: { padding: '0 0 16px' } },
    React.createElement('div', { style: { padding: '16px 24px 16px' } },
      React.createElement('div', { style: { fontFamily: "'Nunito',sans-serif", fontSize: 26, fontWeight: 800, color: '#1E1408' } }, 'Tu progreso')
    ),
    React.createElement('div', { style: { padding: '0 16px', display: 'flex', gap: 10, marginBottom: 14 } },
      React.createElement('div', { style: { flex: 1, background: 'white', borderRadius: 16, padding: 14, boxShadow: '0 2px 12px rgba(30,20,8,0.07)', textAlign: 'center' } },
        ...
        React.createElement('div', { style: { fontSize: 11, color: '#7A6652', marginTop: 2, fontWeight: 500 } }, 'kcal promedio')
      ),
      React.createElement('div', { style: { flex: 1, background: 'white', borderRadius: 16, padding: 14, boxShadow: '0 2px 12px rgba(30,20,8,0.07)', textAlign: 'center' } },
        ...
        React.createElement('div', { style: { fontSize: 11, color: '#7A6652', marginTop: 2, fontWeight: 500 } }, 'días en meta')
      ),
      React.createElement('div', { style: { flex: 1, background: 'white', borderRadius: 16, padding: 14, boxShadow: '0 2px 12px rgba(30,20,8,0.07)', textAlign: 'center' } },
        React.createElement('div', { style: { fontFamily: "'Nunito',sans-serif", fontSize: 26, fontWeight: 900, color: '#FFAB5E' } }, daysLogged),
        React.createElement('div', { style: { fontSize: 11, color: '#7A6652', marginTop: 2, fontWeight: 500 } }, 'días registrados')
      )
    ),
    ...
    React.createElement('div', { style: { padding: '0 16px' } },
      React.createElement('div', { style: { fontFamily: "'Nunito',sans-serif", fontSize: 16, fontWeight: 700, color: '#1E1408', marginBottom: 10 } }, 'Historial reciente'),
      historyEntries.length > 0
        ? historyEntries.map((e, i) => React.createElement(HistoryRow, { key: i, entry: e, goal: goalKcal }))
        : React.createElement('div', { style: { background: 'white', borderRadius: 14, padding: '20px', textAlign: 'center', boxShadow: '0 1px 8px rgba(30,20,8,0.06)' } },
            React.createElement('div', { style: { fontSize: 28, marginBottom: 8 } }, '📊'),
            React.createElement('div', { style: { fontSize: 14, color: '#7A6652' } }, 'Aún no hay datos esta semana'),
            React.createElement('div', { style: { fontSize: 12, color: '#9A8878', marginTop: 4 } }, 'Empieza a registrar comidas para ver tu progreso')
          )
    )
  );
}
