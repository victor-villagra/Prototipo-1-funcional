...
function QuantitySheet({ food, onAdd, onClose }) {
  ...
  return React.createElement('div', {
    ...
    React.createElement('div', { style: { fontFamily: "'Nunito',sans-serif", fontSize: 17, fontWeight: 800, color: '#1E1408', marginBottom: 2 } }, food.name),
    React.createElement('div', { style: { fontSize: 13, color: '#7A6652', marginBottom: 20 } }, isGrams ? 'Ingresa la cantidad en gramos' : 'Ingresa la cantidad en unidades'),
    React.createElement('div', { style: { textAlign: 'center', marginBottom: 8 } },
      ...
      React.createElement('div', { style: { fontSize: 13, color: '#7A6652', marginTop: 4 } }, `kcal · ${displayProtein}g proteína`)
    ),
    React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 12, justifyContent: 'center', marginBottom: 20 } },
      React.createElement('button', {
        ...
        style: { width: 44, height: 44, borderRadius: 999, border: '1.5px solid #EAE0D0', background: 'white', fontSize: 22, cursor: 'pointer', fontWeight: 700, color: '#1E1408', display: 'flex', alignItems: 'center', justifyContent: 'center' }
      }, '−'),
      React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 6 } },
        React.createElement('input', {
          type: 'number', value: qty, onChange: handleInput,
          style: { width: 72, textAlign: 'center', fontFamily: "'Nunito',sans-serif", fontSize: 26, fontWeight: 800, color: '#1E1408', border: '1.5px solid #EAE0D0', borderRadius: 12, padding: '8px 6px', outline: 'none' }
        }),
        React.createElement('span', { style: { fontSize: 14, color: '#9A8878', fontWeight: 500 } }, isGrams ? 'g' : 'u')
      ),
      React.createElement('button', {
  ...
  return React.createElement('div', {
    ...
    React.createElement('div', { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 0 12px' } },
      React.createElement('div', null,
        mealName && React.createElement('div', { style: { fontSize: 11, color: '#9A8878', fontWeight: 500, marginBottom: 2 } }, 'Comida'),
        React.createElement('div', { style: { fontFamily: "'Nunito',sans-serif", fontSize: 17, fontWeight: 800, color: '#1E1408' } }, mealName || 'Tu registro'),
      ),
    ...
    items.map((item, i) =>
      React.createElement('div', {
        key: i,
        style: { display: 'flex', alignItems: 'center', gap: 10, padding: '10px 0', borderBottom: '1px solid #F5EFE4' }
      },
        React.createElement('div', { style: { flex: 1 } },
          React.createElement('div', { style: { fontSize: 14, fontWeight: 600, color: '#1E1408' } }, item.name),
          React.createElement('div', { style: { fontSize: 12, color: '#9A8878', marginTop: 1 } }, item.qtyLabel)
        ),
...
function AddMeal({ onBack, onAddMeal, mealName: initialName = '', foodLibrary }) {
  ...
  if (confirmed) {
    ...
    return React.createElement('div', { style: { display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', gap: 16, padding: 24 } },
      ...
      React.createElement('div', { style: { fontFamily: "'Nunito',sans-serif", fontSize: 22, fontWeight: 800, color: '#1E1408', textAlign: 'center' } }, mealName ? `¡${mealName} registrado!` : '¡Comida registrada!'),
      React.createElement('div', { style: { fontFamily: "'Nunito',sans-serif", fontSize: 32, fontWeight: 900, color: '#F5D040' } }, `${totalKcal} kcal`),
      React.createElement('div', { style: { fontSize: 14, color: '#7A6652', textAlign: 'center' } }, `${cart.length} alimentos agregados${mealName ? ` a ${mealName}` : ' a tu diario'}.`),
      React.createElement('button', {
        onClick: onBack,
  ...
  return React.createElement('div', { style: { display: 'flex', flexDirection: 'column', height: '100%', position: 'relative' } },
    React.createElement('div', { style: { padding: '12px 20px 12px', display: 'flex', alignItems: 'center', gap: 12 } },
      ...
      React.createElement('div', { style: { fontFamily: "'Nunito',sans-serif", fontSize: 20, fontWeight: 800, color: '#1E1408', flex: 1 } }, 'Agregar comida'),
    ),
    React.createElement('div', { style: { padding: '0 20px 10px' } },
      React.createElement('div', { style: { marginBottom: 10 } },
        React.createElement('div', { style: { fontSize: 12, fontWeight: 600, color: '#9A8878', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.06em' } }, 'Nombre de la comida'),
        React.createElement('input', {
          value: mealName,
    ...
    React.createElement('div', { style: { padding: '0 20px 8px' } },
      React.createElement('div', { style: { fontSize: 11, fontWeight: 600, color: '#9A8878', textTransform: 'uppercase', letterSpacing: '0.07em' } }, `${filtered.length} alimentos · toca + para agregar`)
    ),
    React.createElement('div', { style: { flex: 1, overflowY: 'auto', padding: '0 20px', paddingBottom: cart.length > 0 ? 90 : 16 } },
      filtered.map(food =>
        React.createElement('div', {
          key: food.id,
          style: {
            background: 'white', borderRadius: 14, padding: '11px 12px',
          ...
          React.createElement('div', { style: { flex: 1 } },
            React.createElement('div', { style: { fontSize: 14, fontWeight: 600, color: '#1E1408' } }, food.name),
            React.createElement('div', { style: { fontSize: 11, color: '#9A8878', marginTop: 2 } },
      ...
      filtered.length === 0 && React.createElement('div', { style: { textAlign: 'center', padding: '32px 16px', color: '#9A8878' } },
        React.createElement('div', { style: { fontSize: 32, marginBottom: 8 } }, allFoods.length === 0 ? '🥗' : '🔍'),
        React.createElement('div', { style: { fontSize: 14, fontWeight: 500 } }, allFoods.length === 0 ? 'Tu lista de alimentos está vacía' : 'No encontramos ese alimento'),
        React.createElement('div', { style: { fontSize: 12, marginTop: 4 } }, allFoods.length === 0 ? 'Ve a Perfil → Mis alimentos para agregar tus alimentos' : 'Puedes agregarlo en "Mis alimentos"')
      )
    ),
